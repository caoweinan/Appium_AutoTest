...
# 2.2 测试用例，执行步骤，先不运行appium，打印Excel流，流程确认
    ...
    def test_step(self):
        ...
        print(run_list)
        print(run_list[0][0])   # 打印列表里第一条测试用例
    # 根据测试用例执行步骤
        # 根据tc_id获取步骤列表
...
# 2.3 执行appium，完成整体流程   setup teardown
            #click
...
# 1、Excel操作步骤与关键字方法一个映射
    # 创建一个yaml文件
    # excel与方法映射
    # 读取配置文件，根据key获取关键字方法(调试文件：Appium_AutoTest/conf/demo.py)
# 2、关键字方法去BaseAction类寻找方法，gettattr内置函数(调试文件：Appium_AutoTest/conf/demo.py)

# getattr(Action(self.driver),操作的方法)
# 1.根据excel操作步骤映射获取字符串格式的方法名称
# 2.判断操作方法存不存在，存在运行，不存在不往下运行
# 3.根据字符串格式的方法名称，getattr去获取对应的函数对象
# 3、方法执行、参数重新调整
    #1.click需要两个参数(by,value)
    #2.text需要三个参数(by,value,send)
    #3.verify_toast参数是个期望结果DATA_EXPECT_RESULT

    #使用**kwargs这种方式用于接收不定数量的关键字参数，字典方式key=value
    #1.创建一个字典，用于存放这些参数
    #2.调整一下方法：关键字方法，参数**kwargs
